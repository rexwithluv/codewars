...
be strings, because of potentially leading '0's. We already prepared
...
def get_pins(observed: str) -> List[str]:
    ...
    if len(observed) == 1:
        ...
        lst = [keypad[i] for i in observed]
        productos = list(it.product(*lst))
        return ["".join(i) for i in productos]
