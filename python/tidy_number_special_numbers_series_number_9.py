...
A **_Tidy number_**  *is a number whose*  **_digits are in
...
~~~if:prolog
* Since prolog doesn't have booleans,  return value should be 1 for
(True) or 0 for (false)
~~~
___
...
**_The number's digits_**    `{ 1 , 2 }`  are *in non-Decreasing Order*
(i.e) *1 <= 2* .
____
...
**_The Number's Digits_**  `{1 , 0, 2, 4}`  are **_not in non-Decreasing
Order_**  as  *0 <= 1* .
...
**_The number's digits_**    `{1 , 3, 5, 7, 9}`  are *in non-Decreasing
Order* .
____
...
**_The number's digits_**    `{2 , 3, 3, 5}`  are *in non-Decreasing
Order* , **_Note_**   *3 <= 3*
