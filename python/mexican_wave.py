...
the wave can instead reflect back and forth through the crowd. When the
gap in seating is narrow, the wave can sometimes pass through it.
...
def wave(people: str) ->List[str]:
    return [people[:position] + i.upper() + people[position + 1:] \
    for position, i in enumerate(people) if i != " "]
