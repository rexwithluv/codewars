...
0` return "" (return `Nothing` in Elm, "nothing" in Erlang).
...
def longest_consec(strarr: List[str], k: int) -> str:
    ...
    for i in range(len(strarr)):
        actual_consec = strarr[i : i + k]
        actual_word = ""

        for i in actual_consec:
            actual_word += i

        lst_words.append(actual_word)

    return max(lst_words, key=len)
