""" FLAMES game is a method to find out the status of a love
relationship. Entering two names will give you the outcome of a
...
Now, write a method, showRelationship() that takes two names, one male
and one female, and that would return the relationship between them.

Example of FLAMES Game Given names are NEIL and MAE, respectively.

1.- E is common on NEIL and MAE.
2.- Removing E from NEIL and MAE, will left NIL and MA on both names,
respectively
3.- NIL have 3 characters and MA have 2 characters, a total of 5
characters
...
def show_relationship(masc: str, fem: str) -> str:
    flames = {
        1: "Friendship",
        2: "Love",
        3: "Affection",
    ...
    between = list(masc + fem)
    total = [
        i
        for i in between
        if (i in masc and i not in fem) or (i in fem and i not in masc)
    ]
    return flames[len(total) % 6]
